...
$app->post('/cto', function ($request, $response) use ($container)  {
    ...
    if($cto->descricao == null || $cto->latitude == null || $cto->portasUsadas == null || $cto->longitude == null || $cto->spliter["idSpliter"] == null || $cto->bairro["idBairro"] == null)
    {
    ...
    $sth->bindParam("portasUsadas", $cto->portasUsadas);//$init);
    $sth->execute();
...
$sql = "SELECT descricao FROM caixaatendimento WHERE descricao = :descricao AND idcaixa <> ".$cto->idCaixa."";
    ...
    $sql = "update caixaatendimento set latitude = :latitude, longitude = :longitude, descricao = :descricao,portasUsadas = :portasUsadas, idSpliter = :idSpliter, idBairro = :idBairro where idCaixa = :id";
    $sth = $this->db->prepare($sql);
    $sth->bindParam("id", $cto->idCaixa);
    $sth->bindParam("latitude", $cto->latitude);
    $sth->bindParam("longitude", $cto->longitude);
    $sth->bindParam("descricao", $cto->descricao);
    $sth->bindParam("portasUsadas", $cto->portasUsadas);
    $sth->bindParam("idSpliter", $cto->spliter["idSpliter"]);
    $sth->bindParam("idBairro", $cto->bairro["idBairro"]);
    $sth->execute();
